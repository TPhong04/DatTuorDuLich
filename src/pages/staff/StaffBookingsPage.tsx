  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {[
          { label: 'Tổng đơn', val: stats.all, sub: 'toàn hệ thống', bg: 'from-slate-700 to-slate-900', num: 'text-white', subCls: 'text-slate-300', icon: '📋' },
          { label: 'Chờ xử lý', val: ((stats as any).new || 0) + ((stats as any).pending || 0), sub: 'cần gọi xác nhận', bg: 'from-orange-500 to-orange-600', num: 'text-white', subCls: 'text-orange-100', icon: '⏳' },
          { label: 'Hành khách', val: stats.pax, sub: 'tổng SL', bg: 'from-cyan-600 to-cyan-800', num: 'text-white', subCls: 'text-cyan-100', icon: '👥' },
          { label: 'Doanh thu', val: formatMoney(stats.rev), sub: 'chưa đơn hủy', bg: 'from-emerald-600 to-emerald-800', num: 'text-white', subCls: 'text-emerald-100', icon: '💰' },
        ].map((c, i) => (
          <div key={i} className={`relative overflow-hidden rounded-3xl p-5 text-white shadow-sm ring-1 ring-black/5 bg-gradient-to-br ${c.bg}`}>
      ...
      {modalOpen ? (
        <div className="fixed inset-0 z-50 flex items-stretch justify-center bg-slate-900/50 p-0 backdrop-blur-sm sm:items-center sm:p-6">
          <div className="flex max-h-full w-full max-w-5xl flex-col overflow-hidden rounded-none bg-white shadow-2xl sm:rounded-3xl">
            <div className="flex items-start justify-between gap-3 bg-gradient-to-r from-slate-800 via-slate-900 to-slate-800 px-6 py-5 text-white">
              <div>
                ...
                {detail ? (
                  <div className="mt-1 flex flex-wrap items-center gap-3">
                    ...
                    <span className={cn('inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-bold ring-1 ring-inset ring-white/20', statusBadge(detail.status).cls.replace('bg-', 'bg-white/15 text-white ring-white/30 ').replace('ring-1 ring-inset ', ''))}>
                ...
                <div className="space-y-5">
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-slate-50/40 p-4">
                      ...
                      <div className="mt-3 grid grid-cols-2 gap-y-1.5 gap-x-3 text-xs">
                        ...
                        {detail.createdBy ? (<><div className="text-slate-500">Tài khoản:</div><div className="font-bold text-blue-700">User #{detail.createdBy}</div></>) : null}
                      </div>
                      {detail.notes ? (
                  ...
                  <div className="rounded-2xl border border-slate-200 p-4">
                    <div className="mb-2 flex items-center justify-between gap-3">
                      <div className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Ghi chú nội bộ (Nhân viên)</div>
