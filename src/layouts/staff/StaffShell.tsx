...
import { Bell, BookOpen, CalendarDays, ClipboardList, FileText, Home, LayoutDashboard, LogOut, MessageSquareHeart } from 'lucide-react'
  ...
  const groups: NavGroup[] = [
    {
      label: 'Tổng quan',
      items: [
        { label: 'Dashboard', to: '/staff', icon: <LayoutDashboard className="h-4 w-4" />, end: true },
        { label: 'Thông báo', to: '/staff/notifications', icon: <Bell className="h-4 w-4" /> },
      ],
    },
    {
      label: 'Vận hành',
    ...
    },
    {
      label: 'Nội dung',
      items: [
        { label: 'Tin tức', to: '/staff/content/posts', icon: <FileText className="h-4 w-4" /> },
        { label: 'Đánh giá', to: '/staff/content/reviews', icon: <MessageSquareHeart className="h-4 w-4" /> },
      ],
    },
  ]
        ...
        <aside className="relative flex h-[calc(100vh-1.5rem)] flex-col overflow-hidden rounded-3xl bg-gradient-to-b from-blue-900 via-blue-800 to-blue-900 p-4 shadow-2xl shadow-blue-900/20 sticky top-3">
          ...
          >
            ...
            <div className="leading-tight">
              <div className="text-[13px] font-bold tracking-wide text-white">Vận hành</div>
              <div className="text-[10px] font-medium uppercase tracking-widest text-blue-200/80">Staff Panel</div>
            </div>
          </Link>
          ...
          <div className="relative mt-4 space-y-2 border-t border-white/10 pt-3">
            ...
            >
              ...
              Đăng xuất
              <span className="ml-1 text-[10px] text-white/80">({user?.name ?? 'Staff'})</span>
            </button>
          </div>
        </aside>
        ...
        <main className="min-w-0">
          <div className="sticky top-3 z-40 mb-4 flex items-center justify-between gap-3 rounded-3xl border border-slate-200 bg-white/80 px-4 py-3 shadow-sm backdrop-blur lg:px-6">
            <div className="min-w-0">
              <div className="text-[11px] font-semibold uppercase tracking-widest text-slate-400">Hệ thống vận hành</div>
              <div className="truncate text-sm font-bold text-slate-800">Xin chào, {user?.name ?? 'Nhân viên'} 👋</div>
            </div>
